...
/**
 * Trust origins that match this regular expression.
 *
 * @param {String | RegExp} cert
 * @return {Request} for chaining
 * @api public
 */

Request.prototype.trustOrigins = function(regex){
  this._trustOrigins = regex;
  return this;
};
...
Request.prototype._redirect = function(res){
  ...
  const originTrusted = parse(url).host !== parse(this.url).host ||
        (this._trustOrigins && url.search(this._trustOrigins) !== -1);

  // implementation of 302 following defacto standard
  if (res.statusCode == 301 || res.statusCode == 302){
    // strip Content-* related fields
    // in case of POST etc
    headers = utils.cleanHeader(this.req._headers, originTrusted);
